...
import { Plus, Trash2, Pencil, Loader2, Briefcase, GraduationCap, Calendar, MapPin, X, Save } from "lucide-react";
...
export default function ExperienceEducationAdmin() {
    ...
    const handleAddExp = async (e) => {
        ...
        try {
            ...
            if (editingExp) {
                await authFetch(`/api/experience/${editingExp._id}`, { method: "PUT", body: JSON.stringify(payload) });
            } else {
            ...
            setExpForm({ role: "", company: "", location: "", startDate: "", endDate: "", current: false, description: "", keyAchievement: "" });
            fetchData();
        } catch (err) { alert(`Failed to ${editingExp ? 'update' : 'add'} experience`); }
    };
    ...
    const handleAddEdu = async (e) => {
        ...
        try {
            ...
            if (editingEdu) {
                await authFetch(`/api/education/${editingEdu._id}`, { method: "PUT", body: JSON.stringify(payload) });
            } else {
            ...
            setEduForm({ degree: "", institution: "", location: "", startDate: "", endDate: "", grade: "", description: "" });
            fetchData();
        } catch (err) { alert(`Failed to ${editingEdu ? 'update' : 'add'} education`); }
    };
        ...
        <div className="space-y-12">
            ...
            <section className="space-y-6">
                <div className="flex justify-between items-center">
                    ...
                    <button onClick={() => handleOpenExpModal()} className="text-sky-400 border border-sky-400/20 px-4 py-1.5 rounded-lg text-sm font-bold hover:bg-sky-400/10 transition-all flex items-center gap-2">
                    ...
                    {experiences.map(exp => (
                        <div key={exp._id} className="bg-[#1e293b]/50 border border-white/5 p-6 rounded-2xl flex justify-between group">
                            ...
                            <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all">
                                <button onClick={() => handleOpenExpModal(exp)} className="p-2 text-gray-500 hover:text-sky-400">
                                    <Pencil className="w-5 h-5" />
                                </button>
                                <button onClick={() => deleteItem('experience', exp._id)} className="p-2 text-gray-500 hover:text-red-400">
            ...
            <section className="space-y-6">
                <div className="flex justify-between items-center">
                    ...
                    <button onClick={() => handleOpenEduModal()} className="text-purple-400 border border-purple-400/20 px-4 py-1.5 rounded-lg text-sm font-bold hover:bg-purple-400/10 transition-all flex items-center gap-2">
                ...
                <div className="space-y-4">
                    {education.map(edu => (
                        <div key={edu._id} className="bg-[#1e293b]/50 border border-white/5 p-6 rounded-2xl flex justify-between group">
                            ...
                            <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all">
                                <button onClick={() => handleOpenEduModal(edu)} className="p-2 text-gray-500 hover:text-purple-400">
                                    <Pencil className="w-5 h-5" />
                                </button>
                                <button onClick={() => deleteItem('education', edu._id)} className="p-2 text-gray-500 hover:text-red-400">
                                    <Trash2 className="w-5 h-5" />
                                </button>
                            </div>
                        </div>
                ...
                {expModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        <motion.form initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="relative bg-[#1e293b] p-8 rounded-3xl w-full max-w-lg space-y-4" onSubmit={handleAddExp}>
                            <h2 className="text-xl font-bold mb-4">{editingExp ? 'Edit' : 'Add'} Work Experience</h2>
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <input placeholder="Location" className="col-span-2 bg-black/20 border border-white/10 rounded-xl px-4 py-2" value={expForm.location} onChange={e=>setExpForm({...expForm, location: e.target.value})} required/>
                            ...
                            <button type="submit" className="w-full bg-sky-500 py-3 rounded-xl font-bold">{editingExp ? 'Update' : 'Save'} Experience</button>
                        </motion.form>
                    </div>
                )}
                {eduModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        <motion.form initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="relative bg-[#1e293b] p-8 rounded-3xl w-full max-w-lg space-y-4" onSubmit={handleAddEdu}>
                            <h2 className="text-xl font-bold mb-4">{editingEdu ? 'Edit' : 'Add'} Education</h2>
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <input placeholder="Location" className="col-span-2 bg-black/20 border border-white/10 rounded-xl px-4 py-2" value={eduForm.location} onChange={e=>setEduForm({...eduForm, location: e.target.value})} required/>
                                ...
                                <textarea placeholder="Description (Optional)" className="col-span-2 bg-black/20 border border-white/10 rounded-xl px-4 py-2 h-24" value={eduForm.description} onChange={e=>setEduForm({...eduForm, description: e.target.value})} />
                            </div>
                            <button type="submit" className="w-full bg-purple-500 py-3 rounded-xl font-bold">{editingEdu ? 'Update' : 'Save'} Education</button>
                        </motion.form>
                    </div>
                )}
            </AnimatePresence>
